...
from sklearn.utils.validation import check_array, check_random_state, FLOAT_DTYPES, as_float_array, check_is_fitted
...
def k_means_constrained(X, cost, n_clusters, size_min=None, size_max=None, init='k-means++',
    ...
    if n_jobs == 1:
        ...
        results = Parallel(n_jobs=n_jobs, verbose=0)(
            delayed(kmeans_constrained_single)(X, cost, n_clusters,
                                               size_min=size_min, size_max=size_max,
...
def kmeans_constrained_single(X, cost, n_clusters, size_min=None, size_max=None,
    ...
    for i in tqdm(range(max_iter)):
        ...
        labels, inertia = \
            _labels_constrained(X, cost, centers, size_min, size_max, distances=distances)
...
def _labels_constrained(X, cost, centers, size_min, size_max, distances):
    ...
    """
    C = centers
    print("begin labels constrained")
    ...
    edges, costs, capacities, supplies, n_C, n_X = \
        weighted_minimum_cost_flow_problem_graph(X, cost, C, D, size_min, size_max)
    ...
    print("finish labels constrained")

    return labels, inertia

def weighted_minimum_cost_flow_problem_graph(X, bi_cost, C, D, size_min, size_max):
    ...
    scale = 100
    bi_cost = np.array(bi_cost) * scale
    bi_cost[bi_cost==0] = 1

    D = D * scale / bi_cost.reshape(-1, 1)
    ...
    capacities_C_dummy_C = size_max * scale * np.ones(n_C)
    cap_non = bi_cost.sum()  # The total supply and therefore wont restrict flow
    capacities = np.concatenate([
        # np.ones(edges_X_C_dummy.shape[0]),
        bi_cost.reshape(-1,1).repeat(axis=1, repeats=n_C).reshape(-1),
    ...
    # Sources and sinks
    size_min = 0
    supplies_X = bi_cost # np.ones(n_X)
    supplies_C = -1 * size_min * np.ones(n_C)  # Demand node
    supplies_art = -1 * (bi_cost.sum() - n_C * size_min)  # Demand node
...
def solve_min_cost_flow_graph(edges, costs, capacities, supplies, n_C, n_X):
    ...
    for i in range(len(edges[:,0])):
        min_cost_flow.AddArcWithCapacityAndUnitCost(
            int(edges[i,0]), int(edges[i, 1]), int(capacities[i]), int(costs[i]))

    # # Add node supplies
    # min_cost_flow.SetNodeSupplyVectorized(np.arange(N_nodes, dtype='int32'), supplies)

    Nodes = np.arange(N_nodes, dtype="int32")
    for i in range(len(supplies)):
        min_cost_flow.SetNodeSupply(int(Nodes[i]), int(supplies[i]))
    ...
    for i in range(min_cost_flow.NumArcs()):
        labels_M.append(min_cost_flow.Flow(i))
    labels_M = np.array(labels_M[:n_X*n_C]).reshape(n_X, n_C)
...
class KMeansConstrained():
    ...
    def fit(self, X, cost):
        ...
        self.cluster_centers_, self.labels_, self.inertia_, self.n_iter_ = \
            k_means_constrained(
                X, cost, n_clusters=self.n_clusters,
    ...
    def fit_predict(self, X, cost):
        ...
        labels : array, shape [n_samples,]
            Index of the cluster each sample belongs to.
        """
        return self.fit(X, cost).labels_
