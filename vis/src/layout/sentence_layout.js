...
function paintSentenceDAG(self) {
    ...
    const data = raw.lines.map((d, k) => {
        ...
        let last_label = 0
        let last_display = 0
        let threshold = 1.0 / d.line.length / 2.5
    ...
    const widthScale = d3.scalePow().exponent(2)
        .domain(d3.extent([].concat(...data.map(d => d.line.map(n => n.size)))))
        .range([1.5, 9])
    const contriScale = (() => {
        let contris = [].concat(...data.map(d => d.line.map(e => e.contri)))
        contris.sort((a,b) => a - b)
        let n = contris.length
        let left = contris[0]
        let right = contris[n - 1]
        let mid = 0
    ...
    function LineChart() {
        ...
        for (let j = 1, last = 0; j <= n_cols; ++j) {
            if (j < n_cols && data[j].line[i].label == data[j - 1].line[i].label && data[j].line[i].display) {
            continue
            } /*else if (j + 1 < n_cols && data[j + 1].line[i].label == data[j - 1].line[i].label && data[j + 1].line[i].display) {
            j += 1
            continue
            } */ else {
            let curr = j - 1
            if (curr - last >= 1) {
                let w1 = widthScale(data[last].line[i].size)
                let w2 = widthScale(data[curr].line[i].size)
                let y1 = yScale(data[last].line[i].position) - w1 * 0.5 - 12
                let y2 = yScale(data[curr].line[i].position) + w2 * 0.5 + 12
                //console.log(i, last, curr)
                if (true || check_continuity(i + 1, last, curr)) {
                let w3 = widthScale(data[last].line[i + 1].size)
                let w4 = widthScale(data[curr].line[i + 1].size)
                let y3 = yScale(data[last].line[i + 1].position) - w3 * 0.5 - 12
                let y4 = yScale(data[curr].line[i + 1].position) + w4 * 0.5 + 12
                let contri = 0
                let last_contri = 0
                for (let k = last; k <= curr; ++k) {
                    contri += data[k].line[i + 1].contri
                    last_contri += data[k].line[i].contri
                }
                contri /= (curr - last + 1)
                last_contri /= (curr - last + 1)
                for (let k = last; k <= curr; ++k) {
                    last_contri_record[(i + 1) * data.length + k] = contri
                }
                for (let k = last; k <= curr; ++k) {
                    if (last_contri_record[i * data.length + k]) {
                    last_contri = last_contri_record[i * data.length + k]
                    break
                    }
                }
                ...
                shades.push({
                    ...
                    //opacity: 0.2,
                    x: i,
                    y: data[curr].line[i].label,
                })
                }
            }
            last = j
            //if (last < data.length && last > 0 && !data[last].line[i].display && data[last - 1].line[i].display && data[last].line[i].label == data[last - 1].line[i].label) {
            //  last -= 1
            //} else if (last < data.length && !data[last].line[i].display) ++last
            }
        }
        }
        // console.log(shades)
        ...
        for (let k = 0; k + 2 < n_rows; ++k) {
            ...
            if (flag) {
                let y1 = shades[i].y1
                let y2 = shades[i].y2
                let contri = data[shades[i].top].line[k + 1].contri
                let y0 = yScale((data[shades[i].top].line[k].position + data[shades[i].bottom].line[k].position) * 0.5)
        ...
        let top_size = data.map(d => d.line[d.line.length - 1].size).sort((a, b) => b - a)
        top_size = top_size.length > 3 ? top_size[3] : top_size[top_size.length - 1] - 1e-6
                                                    
        const phrase_data = phrases.filter(d => d.type == 'phrase' && d.layer < data[0].line.length)
                        .filter(d => {
                            ...
                            return (data[k].line[d.layer].display && data[k + 1].line[d.layer].display)
                        })
                        
        
        const relation_data = phrases
        .filter(d => d.type == 'relation')
        .filter(d => data[d.top].line[d.layer].display && data[d.bottom].line[d.layer].display)
        ...
        .attr('d', d => {
            ...
            let y = yScale((data[k].line[d.layer - 1].position + data[k + 1].line[d.layer - 1].position) / 2)
                        //data[k].line[d.layer].position + data[k + 1].line[d.layer].position) / 4)
        ...
        for (let j = 0; j < n_cols; ++j) {
            //let a = distScale(data[j].line[i].contri)
            //let b = distScale(data[j].line[Math.max(0, i - 3)].contri)
            let a = data[j].line[i].contri
            let b = data[j].line[0].contri
            let c = data[j].line[n_rows - 2].contri
            if (!data[j].line[i].display) continue
            let left = Math.abs(distScale(b) - distScale(a))
            let right = Math.abs(distScale(c) - distScale(b))
            // console.log(left, right, a, b, c)
            rsegs.push({
            x: i, y: j,
            d: distScale(data[j].line[i - 1].contri) - distScale(data[j].line[i + 1].contri),
            left: data[j].line[i - 1].contri,
            right: data[j].line[i + 1].contri,
        ...
        if (rsegs[i].x >= data[0].line.length - 3) {
        ...
        rsegs = rsegs.map(d => ({ x: xScale(d.x), y: yScale(data[d.y].line[d.x].position), row: d.y, col: d.x, delta: d.d, direction: d.direction }))
        // console.log('rsegs', rsegs)
        /*
        rsegs.forEach(d => {
        let label = data[d.row].line[d.col].label
        let weight = edges[d.col - 1][label].slice(1, 1 + data.length)
        let pl = Math.max(1, d.col - 3)
        let plabel = data[d.row].line[pl].label
        // console.log(pl, plabel, edges[pl].length)
        let pweight = edges[pl - 1][plabel].slice(1, 1 + data.length)
        d.changes = weight.map((e, i) => [i, e - pweight[i]]).sort((a, b) => Math.abs(b[1]) - Math.abs(a[1])).slice(0, 4)
        d.changes = d.changes.sort((a, b) => a[0] - b[0]).map(e => [data[e[0]].text, e[1]])
        ...
        for (let d of rsegs){ data[d.row].line[d.col].has_rseg = 1 }
        for (let i = 1; i < data.length; ++i) {
        let last = -1
        for (let j = 0; j < data[0].line.length; ++j) {
            if (j - last > 4) {
            data[i].line[j].show_label = 1
            }
            if (data[i].line[j].show_label) {
            if (j + 2 >= data[i].line.length) {
                data[i].line[j].show_label = 0
            } else if (j < last) {
                data[i].line[j].show_label = 0
            } else if (j - last <= 3) {
                data[i].line[j].show_label = 0
                if (last + 3 < data[i].line.length) {
                data[i].line[last + 3].show_label = 1
                }
                last = last + 3
            } else if (data[i].line[j].has_rseg) {
                data[i].line[j].show_label = 0
                if (j + 1 < data[i].line.length) {
                data[i].line[j + 1].show_label = 1
                }
            } else if (data[i].line[j + 1].has_rseg) {
                data[i].line[j].show_label = 0
                if (j + 2 < data[i].line.length) {
                data[i].line[j + 2].show_label = 1
                }
            } else {
                last = j
            }
            //if (data[i].line[j + 1].has_rseg);
            //if (data[i - 1].line[j].show_label);
            }
        }
        }
        let annotations = []
        ...
        for (let i = d.top; i < d.bottom; ++i) {
            let delta =
                Math.abs(data[i + 1].line[d.layer].position - data[i].line[d.layer].position) + 
                Math.abs(data[i + 1].line[d.layer + 1].position - data[i].line[d.layer + 1].position)
            if (delta > max_delta) {
            max_delta = delta
            k = i
            }
        }
        let top = Math.max(data[k + 1].line[d.layer].position, data[k + 1].line[d.layer + 1].position)
        let bottom = Math.min(data[k].line[d.layer].position, data[k].line[d.layer + 1].position)
        annotations.push({
            x: xScale(d.layer) + 25,
        ...
        .attr('ry', 2)
        .style('stroke', d => {
            let p = distScale(data[d.row].line[d.col].contri)
        ...
        .style('fill', d => contriScale(data[d.row].line[d.col + 1].contri))
        ...
        for (let i = 0; i + 1 < d.line.length; ++i) {
            ...
            let delta = Math.abs(t - b) < 8 ? 1.5 : 1
        ...
        if (index > 0 && data[index - 1].end_y - d.end_y < 10) {
            d.end_y = data[index - 1].end_y - 10
        }
...
export default paintSentenceDAG
