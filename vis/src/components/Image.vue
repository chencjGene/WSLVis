...
export default {
    ...
    methods: {
        ...
        update_data() {
            ...
            if (that.mode === "grid") {
                ...
                if (aspect_ratio > that.aspect_ratio) {
                    ...
                    width =
                        (that.layout_height * that.focus_image.w) /
                        that.focus_image.h;
                ...
                if (aspect_ratio > that.aspect_ratio) {
                    height = that.layout_height;
                    width =
                        (that.layout_height * that.focus_text.w) /
                        that.focus_text.h;
                    image_x = (that.layout_width - width) / 2;
                    image_y = 0;
                } else {
            ...
            if (that.mode === "grid") {
                ...
                that.one_images
                    ...
                    .attr(
                        ...
                        (d) =>
                            that.server_url +
                            `/image/origin_image?filename=${d.idx}.jpg`
                    )
                    .on("click", (_, d) => {
                ...
                that.one_image_boxes
                    ...
                    .on("click", function (event, d) {
                        ...
                        that.selectClass =
                            that.$store.state.classNames[d.label];
                        // add dragbar
                        that.main_group.selectAll("#drag-bar-g").remove();
                        let new_g = that.main_group
                            .append("g")
                            .attr("id", "drag-bar-g");
                    ...
                    .attr(
                        ...
                        (d, i) =>
                            ...
                            (that.detail_pos !== -1 &&
                                Math.floor(i / that.x_grid_num) >
                                    Math.floor(
                                        that.detail_pos / that.x_grid_num
                                    )) *
        ...
        show_detail(d, i) {
            ...
            if (i === -1) {
                ...
                let img_url =
                    ...
                    ":" +
                    that.Port +
                    `/image/origin_image?filename=${d.id}.jpg`;
                ...
                if (that.detail_pos === -1) {
                    ...
                    that.grid_boxes = that.grid_box_group
                        .selectAll(".info-box")
                        .data(
                            d.boxes.map((_d) => {
                                return {
        ...
        change_grid_page(direction) {
            ...
            if (start_num >= 0 && start_num < that.grid_images.length) {
                ...
                that.grid_images_showing = that.grid_images.slice(
                    start_num,
                    end_num
                );
                that.update_view();
            ...
            let menuOptions = [
                {
                    title:
                        selectNode == null
                            ? "Confirm"
                            : "Confirm as " +
                              that.$store.state.classNames[selectNode],
                    action: function () {
                        console.log("Confirm!");
    ...
    async mounted() {
        ...
        if (
            ...
            that.grid_box_group.attr(
                "transform",
                `translate(${delta_x / 2},0)`
            );
